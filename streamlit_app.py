...
def render_admin_dashboard(judge_email: str) -> None:
    ...
    st.markdown('<div class="section-header">Final Rankings — 60% Judges + 40% Automatic Correctness</div>', unsafe_allow_html=True)
    st.caption(
        "Final Score = 60% × average judge score (scaled to 100) + 40% × Automatic Correctness Score /100. "
        "Missing scores count as 0 — make sure judges finish reviews and correctness scores are calculated for accurate rankings."
    )
    if project_scores.empty:
        st.caption("No submissions available yet.")
    else:
        ranking = project_scores[
            ["Member / Team", "Submitter", "Avg Score /20", "Automatic Correctness Score /100", "Reviews Completed"]
        ].copy()
        ranking["Judge Score /100"] = (
            pd.to_numeric(ranking["Avg Score /20"], errors="coerce") / PIH_SCORE_MAX * 100.0
        ).round(1)
        ranking["Automatic Correctness Score /100"] = pd.to_numeric(
            ranking["Automatic Correctness Score /100"], errors="coerce"
        ).round(1)

        ranking["Final Score /100"] = (
            0.6 * ranking["Judge Score /100"].fillna(0.0)
            + 0.4 * ranking["Automatic Correctness Score /100"].fillna(0.0)
        ).round(1)
