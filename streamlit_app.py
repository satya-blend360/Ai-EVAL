...
def render_admin_dashboard(judge_email: str) -> None:
    ...
    st.markdown('<div class="section-header">Final Rankings — 60% Judges + 40% AI Semantic Score</div>', unsafe_allow_html=True)
    st.caption(
        "Final Score = 60% × average judge score (scaled to 100) + 40% × AI semantic correctness score. "
        "Missing scores count as 0 — run '⚡ Batch Compute All AI Scores' and ensure judges finish reviews for accurate rankings."
    )
    if project_scores.empty:
        ...
        if not submissions.empty and "project_name" in submissions.columns and "ai_score" in submissions.columns:
            ...
            ranking["AI Semantic Score /100"] = pd.to_numeric(
                ranking["Member / Team"].map(ai_by_project), errors="coerce"
            ).round(1)
        else:
            ranking["AI Semantic Score /100"] = np.nan

        ranking["Final Score /100"] = (
            0.6 * ranking["Judge Score /100"].fillna(0.0)
            + 0.4 * ranking["AI Semantic Score /100"].fillna(0.0)
        ).round(1)
